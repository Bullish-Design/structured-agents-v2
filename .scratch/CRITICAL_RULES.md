...
Use kebab-case for directory names (e.g. `v04-refactor`, `litellm-integration`).
...
ALL scratch notes, working files, ad-hoc explorations, and temporary analysis for a project go inside that project's directory — never loose in `.scratch/`. Name them descriptively (e.g. `litellm-spike.md`, `event-migration-notes.md`). The standard files above are the convention; additional files are encouraged whenever they help preserve context.
...
`REPO_RULES.md` has repo-specific standards on top of these.
