...
class Backend:
    ...
    def build[T](self, spec: AgentSpec[T]) -> Agent[T]:
        ...
        return Agent(
            spec,
            DBOSAgent(
                PydanticAgent(  # type: ignore[no-matching-overload]
                    model,
...
class Agent[T]:
    def __init__(self, spec: AgentSpec[T], raw: DBOSAgent[Any, Any]) -> None:
        ...
        @DBOS.workflow(name=f"{spec.name}.structured_run")
        async def structured_run(prompt: str) -> T:
            result = await raw.dbos_wrapped_run_workflow(prompt, deps=None)
            return spec.constraint.parse(result.output)

        self._workflow = structured_run

    async def run(self, prompt: str) -> T:
        return await self._workflow(prompt)
    ...
    def workflow(self) -> Any:
        """The registered durable workflow returning the validated output type."""
