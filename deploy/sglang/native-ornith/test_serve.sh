...
MODEL_PATH="$tmpdir/model.gguf" TOKENIZER_PATH="$tmpdir" CUDA_VISIBLE_DEVICES=1 \
...
expected=(
  ...
  --load-format gguf --host 127.0.0.1 --port 8003 --served-model-name base
  --context-length 16384 --mem-fraction-static 0.80 --max-running-requests 1
  --cpu-offload-gb 0 --grammar-backend xgrammar --tool-call-parser qwen3_coder
  --reasoning-parser qwen3 --disable-cuda-graph
)
mapfile -t actual <"$tmpdir/args"
[[ "${actual[*]}" == "${expected[*]}" ]]

if MODEL_PATH="$tmpdir/model.gguf" TOKENIZER_PATH="$tmpdir" CUDA_VISIBLE_DEVICES=1 \
  SGLANG_GGUF_CONFIG_PATH="$tmpdir/config.json" SGLANG_CACHE_DIR="$tmpdir/cache" SGLANG_PYTHON="$tmpdir/python" MMPROJ_PATH="$tmpdir/mmproj.gguf" \
  bash "$here/serve.sh" >/dev/null 2>&1; then
  echo "multimodal safety gate unexpectedly passed" >&2
  exit 1
fi
