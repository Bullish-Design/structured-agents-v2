...
def _store_gguf_loaded_weight(
    ...
    # Keep the GGUF tensor on CPU until there is a final GPU destination.
    #
    # Moving it to CUDA here and then materializing ``param`` creates two
    # simultaneous GPU copies of every tensor. Besides needlessly raising the
    # load peak, vLLM records that peak as model memory and consequently leaves
    # no capacity for the KV cache on small GPUs. CUDA ``copy_`` supports a CPU
    # source, so materialize the final parameter first and copy into it directly.
    ...
    if not isinstance(param, UninitializedParameter) and param.data.numel() == 0:
        # This fallback is used by already-materialized parameters. It has no
        # later merged-weight materialization step, so it must own GPU storage.
        param.data = loaded_weight.to(device=param.device)
...
def _gguf_embedding_weight_loader(
    ...
    # Slice on CPU, then copy into the final padded GPU parameter. This avoids
    # holding a full GPU embedding staging tensor alongside the destination.
    loaded_weight = _clone_loaded_weight(loaded_weight)
