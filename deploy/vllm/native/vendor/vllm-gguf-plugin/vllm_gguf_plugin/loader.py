...
def _log_parameter_memory(model: nn.Module) -> None:
    """Log the largest resident parameters when explicitly requested.

    GGUF tensors are often substantially smaller on disk than the complete
    model object.  This diagnostic is deliberately opt-in so it can be used
    to audit loader regressions without changing normal service logging.
    """
    if os.environ.get("STRUCTURED_AGENTS_GGUF_MEMORY_AUDIT") != "1":
        return

    params = sorted(
        (
            (parameter.numel() * parameter.element_size(), name)
            for name, parameter in model.named_parameters()
        ),
        reverse=True,
    )
    total = sum(size for size, _ in params)
    logger.info(
        "GGUF resident parameter audit: %.3f GiB total; largest: %s",
        total / (1024**3),
        ", ".join(
            f"{name}={size / (1024**2):.1f} MiB" for size, name in params[:20]
        ),
    )
